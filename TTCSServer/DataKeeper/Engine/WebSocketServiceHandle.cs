...
namespace DataKeeper.Interface
{
    ...
    {      
        public TcpClient WSClient { get; set; }
        ...
        public void CreateConnectionTimeout()
        {
            Task TTCSTask = Task.Run(() =>
            {
                ...
                {
                    if (Timeout == 0)
                        WebSocketServiceHandle.RemoveClient(this);
    ...
    public class WebSocketServiceHandle
    {
        ...
        internal class MainWebSocketService : WebSocketService
        {
            private readonly IWebSocketLogger _logger;

            public MainWebSocketService(NetworkStream networkStream, TcpClient tcpClient, string header, IWebSocketLogger logger) : base(networkStream, tcpClient, header, true, logger)
            {
                _logger = logger;

                WSConnection ThisConnection = new WSConnection();
                ThisConnection.WSClient = tcpClient;
                ThisConnection.IPAddress = ((IPEndPoint)tcpClient.Client.RemoteEndPoint).Address.ToString();
                ThisConnection.Port = ((IPEndPoint)tcpClient.Client.RemoteEndPoint).Port;
        ...
        internal class MainWebHTTPService : HttpService
        {
            public MainWebHTTPService(Stream stream, string path, string webRoot, IWebSocketLogger logger) : base(stream, path, webRoot, logger)
            {
            ...
            public IService CreateInstance(ConnectionDetails connectionDetails)
            {
                switch (connectionDetails.ConnectionType)
                {
                    case ConnectionType.WebSocket:
                        ...
                        return new MainWebSocketService(connectionDetails.TcpClient.GetStream(), connectionDetails.TcpClient, connectionDetails.Header, _logger);                        
                    case ConnectionType.Http:
                        // this path actually refers to the relative location of some html file or image
                        return new MainWebHTTPService(connectionDetails.Stream, connectionDetails.Path, _webRoot, _logger);
                }

                return new BadRequestService(connectionDetails.TcpClient.GetStream(), connectionDetails.Header, _logger);
            }
        }

        internal class MainServer : WebServer
        {
            public MainServer(IServiceFactory serviceFactory, IWebSocketLogger logger) : base(serviceFactory, logger)
            {
        ...
        public static void CreateConnection(DataGridView ClientGrid, Object ObjMainPage, DataGridView TTCSLogGrid, String SocketServerAddress)
        {
            ...
            WebSocketServiceHandle.ClientGrid = ClientGrid;
            WebSocketServiceHandle.TTCSLogGrid = TTCSLogGrid;
            WebSocketServiceHandle.ObjMainPage = ObjMainPage;

            UserList = DatabaseSynchronization.GetAllUser();

            //FleckLog.Level = LogLevel.Info;
            AllConnection = new ConcurrentDictionary<String, WSConnection>();
            //var server = new WebSocketServiceHandle(SocketServerAddress);
            //String CerPath = "C:\\Users\\AstroNET\\AppData\\Roaming\\letsencrypt-win-simple\\httpsacme-v01.api.letsencrypt.org\\astronet.narit.or.th-all.pfx";
        ...
        private static void ReturnMessage(TcpClient Client, String StationName, String DeviceName, String FieldName, String Value, String DataType, String InformationType, String ReturnMessage, String ReturnResult)
        {
